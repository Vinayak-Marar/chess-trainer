...
/**
 * Jump directly to a specific puzzle number in the current set (used by the "Jump to" input)
 *
 * Puzzle numbers are 1-based on screen (matching the "Puzzle X of Y" display) but the underlying
 * `increment` variable is 0-based, so we convert accordingly.
 */
function jumpToPuzzle() {
	// Nothing to do if there's no puzzle set loaded
	if (!puzzleset || puzzleset.length === 0) {
		return;
	}

	const requested = parseInt($('#jumpToPuzzleInput').val(), 10);

	// Validate the input - must be a number within range
	if (isNaN(requested) || requested < 1 || requested > puzzleset.length) {
		alert('Please enter a puzzle number between 1 and ' + puzzleset.length + '.');
		return;
	}

	// Reset per-puzzle state, same as what loadPuzzle/goToNextPuzzle expect going in
	error = false;
	puzzlecomplete = false;
	setcomplete = false;

	increment = requested - 1;

	loadPuzzle(puzzleset[puzzleOrder[increment]]);
	dataTools.saveItem('puzzleIndex', increment);

	// Keep the "Next" button in the correct state if manual advance is on
...
function gameOver() {
	...
	// Disable the jump-to-puzzle input/button since the set is complete
	$('#jumpToPuzzleInput').prop('disabled', true);
	$('#btn_jumptopuzzle').prop('disabled', true);
...
function initialButtonSetup() {
	...
	// Jump-to-puzzle control starts disabled until a set is actually running (re-enabled in startTest)
	$('#jumpToPuzzleInput').prop('disabled', true).val('');
	$('#btn_jumptopuzzle').prop('disabled', true);
}
...
function startTest() {
	...
	// Enable the jump-to-puzzle input/button now that a set is running
	$('#jumpToPuzzleInput').prop('disabled', false).attr('max', puzzleset.length);
	$('#btn_jumptopuzzle').prop('disabled', false);
...
$(() => {
	...
	$('#btn_jumptopuzzle').on('click', function () {
		jumpToPuzzle();
	});

	// Allow pressing Enter in the jump-to-puzzle box instead of needing to click "Go"
	$('#jumpToPuzzleInput').on('keyup', function (event) {
		if (event.key === 'Enter') {
			jumpToPuzzle();
		}
	});
